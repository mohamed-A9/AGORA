...
export async function proxy(req: NextRequest) {
    ...
    if (path.startsWith('/api/auth')) {
        const limit = checkRateLimit(ip, 'auth', 100, 60 * 1000); // Increased to 100/min to prevent blocking session updates
        if (!limit.allowed) {
    ...
    // ✅ Onboarding & Verification Enforcer
    if (token) {
        const isCompleted = (token as any).isOnboardingCompleted;
        const emailVerified = (token as any).emailVerified;

        if (!isCompleted) {
            // If profile is not complete, FORCE /complete-profile
            // Allow /complete-profile, API calls (for saving), and static assets
            if (path !== "/complete-profile" && !path.startsWith("/api") && !path.startsWith("/_next") && !path.includes(".")) {
                return NextResponse.redirect(new URL("/complete-profile", req.url));
            }
        } else {
            // Profile IS complete. Now check Email Verification.
            if (!emailVerified) {
                // If not verified, FORCE /verify-email
                // Allow /verify-success (bridge page for session update)
                if (path !== "/verify-email" && path !== "/verify-success" && !path.startsWith("/api") && !path.startsWith("/_next") && !path.includes(".")) {
                    return NextResponse.redirect(new URL("/verify-email", req.url));
                }
            } else {
                // Profile Complete && Email Verified.
                // Block /complete-profile and /verify-email
                if (path === "/complete-profile" || path === "/verify-email") {
