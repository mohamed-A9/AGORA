...
export default function VenueCard({ id, name, city, category, rating, imageUrl, ambiance, cuisine }: VenueCardProps) {
    return (
        <Link href={`/venue/${id}`} className="group block h-full">
            <div className="relative h-full overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/10 hover:shadow-2xl hover:border-white/20">
                {/* Image Container */}
                ...
                <div className="p-5">
                    <div className="flex justify-between items-start gap-2">
                        <h3 className="text-lg font-bold text-white group-hover:text-white/90 transition-colors line-clamp-1">{name}</h3>
                    </div>

                    <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-white/50">
                        <span>{city}</span>
                        {(ambiance || cuisine) && <span>•</span>}
                        {ambiance && <span className="text-white/80 font-medium">{ambiance}</span>}
                        {cuisine && <span className="text-white/80 font-medium">{cuisine}</span>}
                    </div>
